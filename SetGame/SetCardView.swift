...
class SetCardView: UIView {
    ...
    private func drawShape(in rect: CGRect) {
        ...
        switch symbol {
        case .oval: path = ovalPath(in: rect)
        case .flash: path = flashPath(in: rect)
        case .romb: path = rombPath(in: rect)
        }
        path.lineWidth = 2.0
        path.stroke()
    ...
    private func rombPath(in rect: CGRect) -> UIBezierPath {
        let romb = UIBezierPath()
        romb.move(to: CGPoint(x: rect.minX, y: rect.midY))
        romb.addLine(to: CGPoint(x: rect.midX, y: rect.minY))
        romb.addLine(to: CGPoint(x: rect.maxX, y: rect.midY))
        romb.addLine(to: CGPoint(x: rect.midX, y: rect.maxY))
        romb.close()
        return romb
    }
    ...
    private struct Colors {
        ...
        static let selected = #colorLiteral(red: 0.1764705926, green: 0.4980392158, blue: 0.7568627596, alpha: 1).cgColor
    ...
    private var interStripes: CGFloat = 5.0
